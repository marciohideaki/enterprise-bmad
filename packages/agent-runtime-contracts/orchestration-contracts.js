...
const SubagentSpawnInputSchema = strictObject({
  ...
  if (input.message.role !== 'user')
    context.addIssue({ code: 'custom', path: ['message', 'role'], message: 'child input must be user-authored' });
});
...
const WorkflowStepSchema = strictObject({
  ...
  if (step.message.role !== 'user')
    context.addIssue({ code: 'custom', path: ['message', 'role'], message: 'workflow step input must be user-authored' });
});
...
const WorkflowDefinitionSchema = strictObject({
  ...
  if (new Set(stepIds).size !== stepIds.length)
    context.addIssue({ code: 'custom', path: ['phases'], message: 'workflow step ids must be globally unique' });
  if (new Set(childIds).size !== childIds.length)
    context.addIssue({ code: 'custom', path: ['phases'], message: 'workflow child session ids must be unique' });
...
const WorkflowResultSchema = strictObject({
  ...
  if (new Set(childIds).size !== childIds.length)
    context.addIssue({ code: 'custom', path: ['children'], message: 'workflow child results must be unique' });
